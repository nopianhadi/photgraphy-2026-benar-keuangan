...
const getStatusClass = (status: string, config: ProjectStatusConfig[]) => {
    ...
    const colorMap: { [key: string]: string } = {
        ...
        '#3b82f6': 'bg-blue-600/20 text-blue-800',
        '#8b5cf6': 'bg-purple-100 text-purple-400',
...
const ClientSatisfactionWidget: React.FC<{ feedback: ClientFeedback[]; handleNavigation: (view: ViewType) => void }> = ({ feedback, handleNavigation }) => {
    ...
    const StarRatingDisplay = ({ rating }: { rating: number }) => (<div className="flex items-center">{[1, 2, 3, 4, 5].map(star => (<StarIcon key={star} className={`w-5 h-5 ${star <= rating ? 'text-yellow-800 fill-current' : 'text-slate-300'}`} />))}</div>);
...
const ConversionFunnelWidget: React.FC<{ leads: Lead[] }> = ({ leads }) => {
    ...
    const stats = [
        { label: 'Total Leads', count: leads.length, color: 'bg-blue-600' },
        { label: 'Diskusi', count: leads.filter(l => l.status === LeadStatus.DISCUSSION || l.status === LeadStatus.CONVERTED).length, color: 'bg-indigo-600' },
        { label: 'Converted', count: leads.filter(l => l.status === LeadStatus.CONVERTED).length, color: 'bg-green-600' },
    ];
...
const Dashboard: React.FC<DashboardProps> = ({ projects, clients, transactions, teamMembers, cards, pockets, handleNavigation, leads, teamProjectPayments, packages, clientFeedback, currentUser, projectStatusConfig, profile, totals }) => {
    ...
    return (
        <div className="space-y-8">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-brand-surface p-5 rounded-2xl border border-brand-border shadow-sm">
                <div>
                    ...
                    <p className="text-xs sm:text-sm text-brand-text-secondary mt-0.5">
                        Pusat kendali operasional, agenda pernikahan, dan kesehatan finansial.
                    </p>
                </div>
                <div className="flex items-center gap-2 w-full sm:w-auto">
                    <button
                        onClick={() => handleNavigation(ViewType.PROJECTS)}
                        className="button-primary flex-1 sm:flex-none inline-flex items-center justify-center gap-2 text-xs sm:text-sm py-2 px-3.5"
                    >
                    ...
                    <button
                        ...
                        className="button-secondary flex-1 sm:flex-none inline-flex items-center justify-center gap-2 text-xs sm:text-sm py-2 px-3.5"
                    >
            ...
            {/* 4 Core Primary Vital KPI Cards */}
            ...
            <div className="bg-brand-surface/80 p-4 rounded-2xl border border-brand-border shadow-sm">
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs">
                    <button
                        onClick={() => handleNavigation(ViewType.CALENDAR)}
                        className="flex items-center gap-3 p-2.5 rounded-xl bg-brand-bg/60 hover:bg-brand-bg border border-brand-border/40 transition-all text-left group"
                    >
                        <div className="p-2 rounded-lg bg-amber-500/10 text-amber-400 group-hover:scale-105 transition-transform flex-shrink-0">
                            <CalendarIcon className="w-4 h-4" />
                        </div>
                        <div className="truncate">
                            <p className="text-[10px] uppercase font-bold text-brand-text-secondary tracking-wider">Jadwal Bulan Ini</p>
                            <p className="text-sm font-bold text-brand-text-light">{summary.eventsThisMonth} Acara</p>
                        </div>
                    </button>
                    ...
                    <button
                        ...
                        className="flex items-center gap-3 p-2.5 rounded-xl bg-brand-bg/60 hover:bg-brand-bg border border-brand-border/40 transition-all text-left group"
                    >
                        <div className="p-2 rounded-lg bg-rose-500/10 text-rose-400 group-hover:scale-105 transition-transform flex-shrink-0">
                            <AlertCircleIcon className="w-4 h-4" />
                        </div>
                        <div className="truncate">
                            <p className="text-[10px] uppercase font-bold text-brand-text-secondary tracking-wider">Invoice Belum Lunas</p>
                            <p className="text-sm font-bold text-rose-400">{summary.unpaidInvoices} Invoice</p>
                        </div>
                    </button>
                    ...
                    <button
                        ...
                        className="flex items-center gap-3 p-2.5 rounded-xl bg-brand-bg/60 hover:bg-brand-bg border border-brand-border/40 transition-all text-left group"
                    >
                        <div className="p-2 rounded-lg bg-blue-100 text-blue-800 group-hover:scale-105 transition-transform flex-shrink-0">
                            <BriefcaseIcon className="w-4 h-4" />
                        </div>
                        <div className="truncate">
                            <p className="text-[10px] uppercase font-bold text-brand-text-secondary tracking-wider">Tim & Vendor</p>
                            <p className="text-sm font-bold text-brand-text-light">{summary.totalteamMembers} Orang</p>
                        </div>
                    </button>
                    ...
                    <button
                        ...
                        className="flex items-center gap-3 p-2.5 rounded-xl bg-brand-bg/60 hover:bg-brand-bg border border-brand-border/40 transition-all text-left group"
                    >
                        <div className="p-2 rounded-lg bg-pink-500/10 text-pink-400 group-hover:scale-105 transition-transform flex-shrink-0">
                            <DollarSignIcon className="w-4 h-4" />
                        </div>
                        <div className="truncate">
                            <p className="text-[10px] uppercase font-bold text-brand-text-secondary tracking-wider">Sisa Honor Tim</p>
            ...
            <div className="space-y-4">
                <div className="flex items-center gap-2">
                    ...
                    <h2 className="text-sm font-bold uppercase tracking-wider text-brand-text-secondary">
                        Operasional & Agenda
                    </h2>
                </div>
                <div className="grid grid-cols-1 xl:grid-cols-12 gap-6">
                    <div className="col-span-1 xl:col-span-12 widget-animate" style={{ animationDelay: '300ms' }}>
                        <CalendarMonthWidget projects={projects} profile={profile} handleNavigation={handleNavigation} />
                    </div>
                    <div className="col-span-1 xl:col-span-7 widget-animate" style={{ animationDelay: '350ms' }}>
                        <UpcomingCalendarWidget projects={projects} handleNavigation={handleNavigation} />
                    </div>
                    <div className="col-span-1 xl:col-span-5 widget-animate" style={{ animationDelay: '400ms' }}>
            ...
            <div className="space-y-4">
                <div className="flex items-center gap-2">
                    <DollarSignIcon className="w-4 h-4 text-brand-accent" />
                    <h2 className="text-sm font-bold uppercase tracking-wider text-brand-text-secondary">
                        Kesehatan Finansial & Kas
                    </h2>
                </div>
                <div className="grid grid-cols-1 xl:grid-cols-12 gap-6">
                    <div className="col-span-1 xl:col-span-8 widget-animate" style={{ animationDelay: '450ms' }}>
                        <IncomeChartWidget transactions={transactions} />
                    </div>
                    <div className="col-span-1 xl:col-span-4 widget-animate" style={{ animationDelay: '500ms' }}>
                        <BusinessHealthWidget projects={projects} transactions={transactions} />
                    </div>
                    <div className="col-span-1 xl:col-span-7 widget-animate" style={{ animationDelay: '550ms' }}>
            ...
            <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 xl:grid-cols-12 gap-6">
                    <div className="col-span-1 xl:col-span-4 widget-animate" style={{ animationDelay: '650ms' }}>
                        <LeadSourceWidget leads={leads} />
                    </div>
                    <div className="col-span-1 xl:col-span-4 widget-animate" style={{ animationDelay: '700ms' }}>
                        <ConversionFunnelWidget leads={leads} />
                    </div>
                    <div className="col-span-1 xl:col-span-4 widget-animate" style={{ animationDelay: '750ms' }}>
                        <PackageDistributionWidget projects={projects} />
                    </div>
                    <div className="col-span-1 xl:col-span-4 widget-animate" style={{ animationDelay: '800ms' }}>
                        <LeadsSummaryWidget leads={leads} handleNavigation={handleNavigation} />
                    </div>
                    <div className="col-span-1 xl:col-span-4 widget-animate" style={{ animationDelay: '850ms' }}>
                        <BookingTrendWidget projects={projects} />
                    </div>
                    <div className="col-span-1 xl:col-span-4 widget-animate" style={{ animationDelay: '900ms' }}>
            ...
            <div className="widget-animate" style={{ animationDelay: '950ms' }}>
