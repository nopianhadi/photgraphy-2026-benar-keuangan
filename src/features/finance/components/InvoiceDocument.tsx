          ...
          <table className="w-full text-left table-fixed">
            <thead>
              <tr className="bg-slate-100 border-b border-slate-200 print:bg-slate-50">
                <th className="px-4 py-3 text-center text-[10px] font-black text-slate-600 uppercase tracking-widest w-[10%]">No</th>
                <th className="px-4 py-3 text-[10px] font-black text-slate-600 uppercase tracking-widest w-[60%]">Deskripsi Produk / Layanan</th>
            ...
            <tbody className="divide-y divide-slate-100 print:divide-slate-200 text-[13px]">
              ...
              {project.addOns?.map((addon, idx) => (
                <tr key={addon.id}>
                  <td className="px-4 py-3 text-center text-slate-500 font-medium">{2 + idx}</td>
              ...
              {project.transportCost && Number(project.transportCost) > 0 && (
                <tr>
                  <td className="px-4 py-3 text-center text-slate-500 font-medium">{(project.addOns?.length || 0) + 2}</td>
                  <td className="px-4 py-3">
              ...
              {project.customCosts?.map((cost, idx) => {
                ...
                return (
                  <tr key={cost.id}>
                    <td className="px-4 py-3 text-center text-slate-500 font-medium">{rowNo}</td>
                    <td className="px-4 py-3">
